...
def main():
    ...
    output_file = "repo.lite.json"
    output_file2 = "repo.lite.altstore.json"
    
    # Tạo đường dẫn đầy đủ
    full_path = os.path.join(output_dir, output_file)
    full_path2 = os.path.join(output_dir, output_file2)
    ...
    try:
        ...
        consolidated_data["tintColor"] = "9ac734"
        for app in consolidated_data.get("apps", []):
            app["tintColor"] = "9ac734"
        print(f"🎨 Đã áp dụng tintColor #9ac734 ")
        # Lưu vào file
        with open(full_path, 'w', encoding='utf-8') as f:
            json.dump(consolidated_data, f, ensure_ascii=False, separators=(',', ':'))
        print(f"Đã lưu dữ liệu vào file {full_path}")

        # Lưu vào file altstore
        with open(full_path2, 'w', encoding='utf-8') as f:
            json.dump(transform_data(consolidated_data), f, ensure_ascii=False, separators=(',', ':'))


        print(f"Đã lưu dữ liệu vào file {full_path2}")
